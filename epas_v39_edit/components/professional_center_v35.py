"""EPAS v3.5 professional operations center: SLA, work queue, governance and security assurance."""
...
def render(project_id: str | None = None, include_security: bool = False):
    with st.expander('Professional Operations Center', expanded=False):
        ...
        with tabs[4]: render_phase_and_coordination(project_id)
        with tabs[5]: render_survey_control(project_id)
        if include_security:
            st.markdown('---')
            render_security()
