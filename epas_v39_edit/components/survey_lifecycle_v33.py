...
def render(project_id: str | None = None):
    ...
    st.markdown('### EPAS v3.3 Survey Lifecycle Control')
    ...
    for row in rows:
        ...
        if phase == 'in_service' and role in ('owner','ship_management') and not row.get('rfi_id'):
            ...
            with st.expander('Initiate this In-Service survey cycle', expanded=False):
                survey_type = st.text_input('Survey type', value=row.get('survey_type') or 'In-Service Survey', key=f'v32_type_{row["schedule_id"]}')
                requested = st.date_input('Requested survey date', value=row.get('next_due_date') or date.today(), key=f'v32_date_{row["schedule_id"]}')
                priority = st.selectbox('Priority', ['low','medium','high'], index=1, key=f'v32_pri_{row["schedule_id"]}')
                scope = st.text_area('Survey scope / request', key=f'v32_scope_{row["schedule_id"]}')
                if st.button('Create In-Service RFI for this cycle →', key=f'v32_create_{row["schedule_id"]}', type='primary'):
                    if not scope.strip():
        ...
        if phase == 'in_service' and role in ('gm','dm') and row.get('schedule_config_status') in ('REVIEW_REQUIRED','CONFIGURATION_REQUIRED'):
            with st.expander('Configure In-Service schedule basis', expanded=False):
                ...
                interval = st.number_input('Survey interval (months)', min_value=1, max_value=120, value=interval_default or 1, key=f'v32_interval_{row["schedule_id"]}')
                basis_options = ['CERTIFICATE_EXPIRY','CLASS_SURVEY_PROGRAM','REGULATORY_REQUIREMENT','GM_APPROVED_PROGRAM','MANUAL_APPROVED']
                basis = st.selectbox('Schedule basis', basis_options, index=basis_options.index(row.get('due_basis')) if row.get('due_basis') in basis_options else 0, key=f'v32_basis_{row["schedule_id"]}')
                reference = st.text_input('Basis reference', value=row.get('due_basis_reference') or '', key=f'v32_ref_{row["schedule_id"]}')
                basis_default = row.get('due_basis_date')
                basis_date = st.date_input('Schedule basis date', value=date.fromisoformat(basis_default) if basis_default else date.today(), key=f'v32_basis_date_{row["schedule_id"]}')
                before = st.number_input('Window before due (days)', min_value=1, max_value=365, value=90, key=f'v32_before_{row["schedule_id"]}')
                after = st.number_input('Window after due (days)', min_value=1, max_value=365, value=30, key=f'v32_after_{row["schedule_id"]}')
                if st.button('Save schedule basis →', key=f'v32_save_{row["schedule_id"]}', type='primary'):
...
def render_role_acceptance():
    ...
    if not cases:
        return
    with st.expander('v3.3 Workflow Acceptance Matrix', expanded=False):
        for c in cases:
