"""EPAS v2.0 professional operations center: SLA, work queue, governance and security assurance."""
...
def render_role_header():
    s = _safe(pq.role_dashboard_summary) or {}
    c = st.columns(6)
...
def render_lifecycle(project_id: str | None = None):
    ...
    if project_id:
        with st.expander('Project lifecycle timeline', expanded=False):
            events = _safe(lambda: pq.project_timeline(project_id, 100)) or []
            for e in events:
...
def render_survey_control(project_id: str | None = None):
    ...
    rows = _safe(lambda: pq.survey_control_tower(project_id)) or []
    if not rows:
...
def render(project_id: str | None = None, include_security: bool = False):
    with st.expander('Professional Operations Center', expanded=False):
        render_role_header()
        tab_names = ['Work Queue','SLA','Governance','Lifecycle','Survey Control']
        if include_security:
            tab_names.append('Security')
        tabs=st.tabs(tab_names)
        with tabs[0]: render_work_queue()
        with tabs[1]: render_sla(project_id)
        with tabs[2]:
            if project_id: render_governance(project_id)
            else: st.info('Select a project to view integrated governance records.')
        with tabs[3]: render_lifecycle(project_id)
        with tabs[4]:
            render_survey_control(project_id)
        if include_security:
            with tabs[5]: render_security()
