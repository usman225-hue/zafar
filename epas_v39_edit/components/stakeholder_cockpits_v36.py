"""EPAS v3.6 role-native stakeholder operations surfaces.
...
def owner_fleet():
    ...
    if rows:
        ...
        if filt == 'Survey due': rows = [r for r in rows if str(r.get('survey_status','')).upper() in ('IN_SERVICE_DUE','NSC_IN_PROGRESS') or (r.get('days_to_due') is not None and 0 <= int(r.get('days_to_due') or 99999) <= 90)]
        elif filt == 'Overdue': rows = [r for r in rows if 'OVERDUE' in str(r.get('survey_status','')).upper()]
        elif filt == 'Certificates expiring': rows = [r for r in rows if r.get('certificate_expiry') or r.get('certificate_expiry_date')]
