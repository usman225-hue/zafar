"""PSB EPAS v3.9 — Project-specific role workspace.
...
PROJECT_NAV = [
    ...
    ("Ship Register", "ship_register", "Review-only vessel/class/survey status"),
...
def _overview(role, project, vessel, health):
    """Project-only landing page: no workflow snapshot and no recent activity."""
    ...
    left, mid, right = st.columns([1.25, 1.15, 1.0])
    with left:
        _project_panel_open("PROJECT LIFECYCLE & PHASE", "Plan → NSC → In-Service")
        for p in phase_rows:
            icon = {"COMPLETED":"✓","IN_PROGRESS":"→","READY":"●","LOCKED":"○","BLOCKED":"!"}.get(str(p.get("status","")).upper(),"•")
    ...
    with mid:
        _project_panel_open("MY PRIORITY ACTIONS", "Current project work only")
        rows = _safe(lambda: [r for r in (pq.my_work_queue() or []) if str(r.get("project_id")) == str(project["id"])], "Priority actions") or []
        if not rows:
        ...
        if st.button("Open My Project Work →", key=f"project_work_{role}_{project['id']}"):
            st.session_state["project_nav_key"] = "work"
            st.rerun()
        _project_panel_close()
    with right:
        _project_panel_open("CERTIFICATES OVERVIEW", "Current project certificates")
        certs = _safe(lambda: pq.certificates(project["id"]), "Project certificates") or []
        if not certs:
        ...
        if certs and st.button("View all certificates →", key=f"project_certs_{project['id']}"):
            st.session_state["project_nav_key"] = "certification"
            st.rerun()
        _project_panel_close()

    st.markdown("### Milestone Status")
    milestones = _safe(lambda: pq.project_milestones(project["id"]), "Project milestones") or []
    if not milestones:
        ...
        for m in milestones[:12]:
            ...
            st.caption(f"Due {m.get('due_date') or '—'} · {m.get('gate_note') or ''}")
...
def _ship_register(pid, vessel):
    ...
    rows = _safe(pq.ship_register_rows, "Ship Register") or []
    rows = [r for r in rows if str(r.get("project_id")) == str(pid)]
