"""EPAS v3.7 PSB-reference-aligned role cockpits."""
...
def render(role):
    ...
    s=_summary()
    st.markdown(f"<div class='ref-page-kicker'>PAKISTAN SHIPPING BUREAU · EPAS v3.7</div><h1 class='ref-page-title'>{html.escape(title)}</h1><div class='ref-page-subtitle'>{html.escape(sub)}</div>",unsafe_allow_html=True)
    defaults={
    ...
    with a:
      ...
      _close()
    with b:
      _panel('Workflow Snapshot','Immediate controlled work')
      for label,count,tone in _workflow(role,s):
    ...
    with x:
      ...
      _close()
    with y:
      _panel('Recent Activities','Last controlled events')
      for t,v,ago in [('Drawing REV-04 submitted for review','MV AL-FALAH','15m ago'),('Survey report submitted by Surveyor','MV OCEAN STAR','1h ago'),('Corrective action evidence submitted','MV SEA PRIDE','2h ago')]:
        st.markdown(f"<div class='ref-activity'><span class='activity-dot'></span><div><strong>{t}</strong><span>{v} · {ago}</span></div></div>",unsafe_allow_html=True)
      _close()
    with z:
      _panel('Certificates Issued','Recent controlled certificates')
